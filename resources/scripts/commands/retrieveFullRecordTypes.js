...
exports.run = async function (uriOrNames) {
    try {
        ...
        let recordTypesByObject = metadataFromFileSystem[MetadataTypes.RECORD_TYPE].childs;
        ...
        if (selectedForRetrieve) {
            if (selectedForRetrieve === 'All Record Types') {
                retrieveRecordTypes(user, { retrieveOption: 'all' }, metadataFromFileSystem);
            } else {
                ...
                if (sObjectSelected) {
                    ...
                    if (selectedRecordType === sObjectOption) {
                        retrieveRecordTypes(user, { retrieveOption: 'byObject', sObject: sObjectSelected }, metadataFromFileSystem);
                    } else if (selectedRecordType) {
                        retrieveRecordTypes(user, { retrieveOption: 'single', sObject: sObjectSelected, recordType: selectedRecordType }, metadataFromFileSystem);
                    }
                }
            }
        }
...
function retrieveRecordTypes(user, options, metadata) {
    ...
    retrieveSFDXFormat(user, metadata, options);
}

function retrieveSFDXFormat(user, metadata, options) {
